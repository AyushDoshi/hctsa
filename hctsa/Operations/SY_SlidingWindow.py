...
from hctsa.Operations import EN_ApEn, EN_SampEn, DN_Moments, CO_AutoCorr
...
def SY_SlidingWindow(y, windowStat='mean', acrossWinStat='std', numSeg=5, incMove=2):
    ...
    qs = numpy.zeros(numSteps)

    if windowStat == 'mean':

        for i in range(numSteps):
            qs[i] = numpy.mean(y[getWindow(i)])

    elif windowStat == 'std':

        for i in range(numSteps):
            qs[i] = numpy.std(y[getWindow(i)])


    elif windowStat == 'apen':

        for i in range(numSteps):
            qs[i] = EN_ApEn(y[getWindow(i)])
        ...
        for i in range(numSteps):
            qs[i] = CO_AutoCorr(y[getWindow(i)])

    if acrossWinStat == 'std':

        out = numpy.std(qs) / numpy.std(y)
...
def getWindow(stepInd, inc, winLength):
    """
    """
    return numpy.arange((stepInd - 1) * inc, (stepInd - 1) * inc + winLength)
