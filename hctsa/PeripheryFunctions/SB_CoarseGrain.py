...
def SB_CoarseGrain(y, howtocg, numGroups):
    ...
    if howtocg == 'updown':
        ...
        y = numpy.diff(y)
        N -= 1  # the time series is one value shorter than the input because of differenceing
        howtocg = 'quantile'
        ...
        if numGroups == 'tau':
            tau = CO_FirstZero(y)  # first zero crossing of the autocorrelation data
        else:
            tau = numGroups

        if tau > N / 25:
            tau = int(numpy.floor(N / 25))

        m1 = y[0:(N - tau)]
        m2 = y[tau:N]

        upr = numpy.argwhere(m2 >= 0)
        downr = numpy.argwhere(m2 < 0)
    ...
    if howtocg == 'quantile':
        ...
        th = sc.stats.mstats.mquantiles(y, numpy.linspace(0, 1, numGroups + 1), alphap=0.5,
                                        betap=0.5)  # matlab uses peicewise linear interpolation for calculating quantiles, hence alphap=0.5, betap=0.5
        th[0] -= 1  # ensures the first point is included
        yth = numpy.zeros([N, 1])

        for i in range(0,
                       numGroups):  # had some trouble here finding indexes that satisfy two conditions in python, hence this is written differently then in matlab
